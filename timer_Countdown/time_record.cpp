...
//            setCircleProgressBar(remainingTime,100-remainingTime/StudyDivisor); //这里除以3是必须改的
...
void Time_record::my_init() //进行初始化
{
    //这里设置了也没有用的，因为不能把同一个对象放到不同布局的对应位置上
      ...
      Progess_BarSon=new ProgessSon(Progess_Bar);
...
//      qDebug()<<QDir::currentPath();
      FinishPlayer->setMedia(QUrl::fromLocalFile("FinishMusic.wav")); //初始化会议结束后的内容,如果打包后给别人，则这个文件的相对路径得进行一个改变
...
void Time_record::ui_init()
{
    ...
    ui->comboBox->setView(MyView=new QListView);  //有了这个它的下拉框的1这些东西才能生效
...
{
    qDebug()<<"同步时间追踪,后面要和时间追踪那个模块进行一个关联的";
